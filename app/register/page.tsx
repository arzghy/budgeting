...
function RegisterContent() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) {
        setError("Gagal mendaftar. Silakan coba kembali atau gunakan Google.");
        setLoading(false);
      } else {
        router.push(callbackUrl);
      }
  ...
  return (
    <main className="relative flex min-h-screen w-full overflow-hidden bg-[#fffdf6] text-ink">
      <div className="grid w-full grid-cols-1 lg:grid-cols-12 min-h-screen">
        ...
        {/* ═══ RIGHT SIDE: ELEGANT REGISTER FORM ═══ */}
          ...
          <div className="mx-auto w-full max-w-md my-auto py-6 sm:py-8">
            <div className="text-center sm:text-left">
              ...
              <p className="mt-2 text-sm text-ink/70">
                Buat akun gratis untuk menikmati ruang pencatatan keuangan yang tenang dan estetik.
              </p>
            </div>
            ...
            <form onSubmit={handleRegister} className="mt-6 space-y-3.5">
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    required
                    className="w-full rounded-2xl border-2 border-ink/15 bg-white/90 px-4 py-3 pl-11 text-sm font-medium text-ink transition focus:border-sage focus:bg-white focus:outline-none focus:ring-4 focus:ring-sage/20"
                  />
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    required
                    className="w-full rounded-2xl border-2 border-ink/15 bg-white/90 px-4 py-3 pl-11 pr-11 text-sm font-medium text-ink transition focus:border-sage focus:bg-white focus:outline-none focus:ring-4 focus:ring-sage/20"
                  />
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-base text-ink/40">
                    🔒
                  </span>
              ...
              <button
                type="submit"
                disabled={loading}
                className="group relative mt-2 flex w-full items-center justify-center gap-3 overflow-hidden rounded-full border-2 border-ink bg-gradient-to-br from-sage via-[#cee57a] to-[#b8d462] px-6 py-3.5 font-display text-base font-bold text-ink shadow-[0_5px_0_rgba(74,84,64,0.18)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_7px_0_rgba(74,84,64,0.18)] active:translate-y-0.5 active:shadow-[0_2px_0_rgba(74,84,64,0.18)] disabled:opacity-60"
              >
